...
from task3.task3_constants import EMBEDDING_MODEL
...
class VectorStore:
    ...
    def create_doc_embeddings(self) -> None:
        ...
        for doc_name, doc in self.documents.items():
            response = self.client.embeddings.create(input=doc, model=EMBEDDING_MODEL)
    ...
    def create_input_embeddings(self, message: str) -> list[float]:
        console.print("[bold yellow]System:[/bold yellow] Creating query embedding")

        response = self.client.embeddings.create(input=message, model=EMBEDDING_MODEL)

        return response.data[0].embedding

    def search(self, query_embedding: list[float], top_k: int = 3) -> dict[str, float]:
        ...
        for i, (doc_name, score) in enumerate(results.items(), start=1):
            console.print(f"({i}) {doc_name}: {score}")

        return results

    def create_context(self, leader_scores: dict) -> str:
        console.print(
            "[bold yellow]System:[/bold yellow] Creating context for the query"
        )
    ...
    def create_query_with_context(self, message: str, context: str) -> str:
